...
- The current final-DEX verifier checks byte markers, not structural invocation/count/location, and the build stops at an unsigned APK. Treat its result as a static check rather than release verification.
- The decoded 430 resource table emitted warnings and apktool's rebuilt resource payload was diagnosed as lossy. The final graft deliberately discards all intermediate resources and manifest data; a successful graft still requires zipalign, signing, final signed-APK verification, installed-APK identity checking, and controlled runtime validation.
