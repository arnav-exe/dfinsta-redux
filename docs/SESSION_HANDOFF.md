...
The latest full `unittest` discovery passes 298 tests. Independent adversarial review returned GO after authority-shadowing, opaque source evidence, repeated-cancellation, and cancellation/failure race findings were fixed. No real generic 340/430 apktool replay has run yet. The legacy target-specific rebuilds remain the exercised artifact evidence.

## Immediate Next Actions

1. Admit immutable real apktool/JVM/framework artifacts and exercise 340/430 clean APK decode plus tree apply through the ledger-owned checkpoint path; do not weaken the synthetic authority boundary for convenience.
