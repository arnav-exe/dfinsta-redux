...
The latest full `unittest` discovery passes 319 tests. Independent framework review returned GO after per-install package-ID/overwrite enforcement and apply lifecycle shadowing were fixed. No real generic 340/430 apktool replay has run yet. The legacy target-specific rebuilds remain the exercised artifact evidence.

## Immediate Next Actions

1. Add an opt-in checked-in integration harness that admits immutable real apktool/JVM/framework artifacts and exercises 340/430 clean APK decode plus tree apply through the ledger-owned checkpoint path; do not weaken the authority boundary for convenience.
2. Add mutation fixtures for missing/ambiguous anchors, DEX movement/collision, and unsafe resource rebuilds, then add the ledger-owned build/patched-APK checkpoint.
...
4. Register the admitted decode/apply/build sequence in the durable Workflow only after the complete effect chain and adoption rules are reviewed.
5. Add Google ADK as bounded read-only Temporal Activities only after deterministic generalization passes; specialist topology follows measured failure clusters.
