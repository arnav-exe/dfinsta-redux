...
The atomic commits from `3e91eb5` through `7498dfd` implement, review, and harden the first durable slice. Phase A retains 35 focused tests; the latest full `unittest` discovery after ledger-owned decoded-tree receipts passes 279 tests.
...
Implementation checkpoint: strict contracts, the target-neutral compiler, generated 340/430 fixtures, decoded-tree apply primitives, both archive backends, receipt-bound verification, gate-bound tool admission, role execution plans, replay-v3 admission, append-only ledger authority, replay-v3 secure source staging, hardened CAS blobs, and canonical decoded-tree manifest/blob closures are committed and independently reviewed. The unregistered decode Activity now emits `ReplayDecodedTreeReceiptV1`, binding the exact input APK, admitted profile/plan/capability/tool/request, canonical manifest, semantic hash, and operation key. It validates the complete receipt/manifest/blob closure before effect recording and again during adoption. Output capture opens `output` relative to the still-pinned workspace descriptor, so pathname replacement cannot redirect capture. Active decode claims reject takeover; failures before workspace creation explicitly release only the exact pending claim, while cancellation and workspace-created failures quarantine. Tree capture remains a sequential stability-checked scan of a quiescent owner-private tree; hostile same-UID mutation, descendant confinement, and real apktool remain blocked. Provisioned mini-tree tests apply and reapply all 59 340 and seven 430 operations; real generic replay remains pending.
