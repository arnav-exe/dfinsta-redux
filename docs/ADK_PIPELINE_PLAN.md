...
The atomic commits from `3e91eb5` through `7498dfd` implement, review, and harden the first durable slice. Phase A retains 35 focused tests; the latest full `unittest` discovery after the ledger-owned tree apply checkpoint passes 298 tests.
...
Implementation checkpoint: strict contracts, the target-neutral compiler, generated 340/430 fixtures, decoded-tree apply primitives, both archive backends, receipt-bound verification, gate-bound tool admission, role execution plans, replay-v3 admission, append-only ledger authority, replay-v3 secure source staging, hardened CAS blobs, and canonical decoded-tree manifest/blob closures are committed and independently reviewed. The unregistered decode Activity emits `ReplayDecodedTreeReceiptV1`, binding the exact input APK, admitted profile/plan/capability/tool/request, canonical manifest, semantic hash, and operation key. A second unregistered Activity requires that exact completed ledger output, materializes its closure, stages and re-verifies replay-v3 source, compiles and applies the target-neutral specification, and emits `ReplayPatchedTreeReceiptV1` with reconstructable source-admission evidence, ordered operation outcomes, and a complete patched-tree closure. Both Activities use concrete ledger authority methods, reject overlapping active owners, release only pre-workspace claims, and quarantine partial work. Apply cancellation is supervised across repeated cancellation until the mutation thread stops; capture and effect publication occur only afterward. Output capture uses pinned descriptors so pathname replacement cannot redirect the authoritative scan. Tree capture remains a sequential stability-checked scan of a quiescent owner-private tree; hostile same-UID mutation, hard worker loss, descendant confinement, and real apktool remain blocked. Provisioned mini-tree tests apply and reapply all 59 340 and seven 430 operations; real generic replay remains pending.
