...
The atomic commits from `3e91eb5` through `7498dfd` implement, review, and harden the first durable slice. Phase A retains 35 focused tests. The 319-test figure recorded here was the full-discovery total at framework-cache integration and is historical; see the dated totals later in this document, and treat the newest one as current.
...
- The SQLite ledger records append-only pending, effect, completion, and quarantine events; update/delete triggers prevent history rewriting. A transactional current-claim index fences promotion to its latest owner. Any new Workflow/Activity owner may supersede an abandoned claim only for explicitly retry-safe deterministic CAS effects; non-retry-safe claims reject competitors. A non-retry-safe Activity may explicitly release its exact pending claim only before creating a workspace, allowing a later owner to recover without overlapping execution. Released state currently lives only in the current-claim index and cannot be reconstructed from the append-only event stream alone. Schema creation, upgrade, and legacy event backfill serialize under one immediate transaction.
...
Implementation checkpoint: strict contracts, the target-neutral compiler, generated 340/430 fixtures, decoded-tree apply primitives, both archive backends, receipt-bound verification, gate-bound tool admission, role execution plans, replay-v3 admission, append-only ledger authority, replay-v3 secure source staging, hardened CAS blobs, and canonical decoded-tree manifest/blob closures are committed and independently reviewed. Profiles with frameworks now require an aggregate ledger-owned installer checkpoint: admitted framework APKs run sequentially by package ID, every step must add exactly its declared cache entry without changing prior entries, and the final descriptor-captured cache closure is bound by `ReplayFrameworkCacheReceiptV1`. Framework-bearing decode requires that exact completed predecessor, materializes and verifies the cache, and emits framework-aware `ReplayDecodedTreeReceiptV2`; profiles without frameworks retain the byte-identical V1 identity path. Tree apply accepts either exact completed decode version and emits `ReplayPatchedTreeReceiptV1` with reconstructable source-admission evidence, ordered outcomes, and a complete patched-tree closure. The unregistered build checkpoint requires those exact completed predecessors, executes the admitted build capability unchanged, composes either a full rebuild or stock DEX graft, independently validates exact APK bytes, and emits `ReplayPatchedApkReceiptV1`. All checkpoint Activities use concrete ledger methods, reject overlapping active owners, release only pre-workspace claims, and quarantine partial work. Apply cancellation is supervised across repeated cancellation until mutation stops. Output capture uses pinned descriptors. Hostile same-UID mutation, hard worker loss, and descendant confinement remain blocked.

An independently reviewed opt-in harness has now exercised real apktool 2.9.3 against both clean fixtures through the ledger-owned direct-Activity chain. Two initial 340 runs failed closed and exposed invalid Windows-portability assumptions: real output includes `AUX.smali` and hundreds of case-distinct obfuscated filenames. The corrected Linux-only manifest policy preserves those exact names while retaining exact uniqueness/topology, no-follow capture/materialization, and casefold-collision controls for authored sources and operation destinations. The successful run from commit `58df5ad07ec2989342aeefed59d33e06319820d5` installed the API 36 framework for 430, decoded both APKs, applied all 59/7 operations, captured complete patched closures, and adopted every completed stage without relaunch. Evidence file `/home/arnav/AI/dfinsta-real-replay-340-430-retry-2/success.json` has SHA-256 `840729530a3734d68e89c26c7dddf95f3911b65b427ead5a5bd7caae1aebf925`. This proves mechanical direct-Activity replay only; authenticated authority, build composition, final APK verification, Workflow registration, and runtime behavior remain pending.

The first real 340 build probe then failed closed after apktool exited successfully because the generic executor correctly reported undeclared `framework/1.apk` and `patched-tree/build` outputs. Read-only analysis showed no authoritative patched-tree changes: apktool added exactly `framework/1.apk` plus 15,218 entries below `patched-tree/build`. The reviewed profile policy now admits only those observed build side effects plus `intermediate.apk` for no-framework builds; framework-bearing builds omit `framework/1.apk` authority. The build Activity rejects preexisting generated paths, removes only the admitted cache tree and singly linked framework file descriptor-relatively without following symlinks, preserves declared framework caches, and exact-verifies canonical tree closures before capture. Replacement boundaries fail closed. Full warning-strict discovery passes 391 tests with one expected opt-in integration skip.

The approved 340-only retry from commit `d8d018749f5b501cafda09b0c383ca4f9bcf4342` completed decode, all 59 apply operations, full rebuild, and clean second-run adoption in 932.0 seconds. Canonical evidence `/home/arnav/AI/dfinsta-real-build-retry-340-2/success.json` has SHA-256 `804299c3f94ad28e1e53fee02acc74138ced32412366a3e03138900a4c6cc100`. Its completed build operation `47520686021b1ee4cda9b9c7f2107d6136f58b571df99e9192ccf58cadbfb3f2` emitted receipt `ba41ded768688ea7604a9a88c8d55431fdac40f72543195c289106e862ff73eb`; intermediate and final full-rebuild APK bytes are identical at SHA-256 `7f84ee75c8d09ab0d18c91a1e1f152d71be1377c623eba2bb64422b6a88f6f35`, size 87,860,247, with the expected 11 DEX entries. An independent audit found every one of 129 reachable producer pairs asserted and completed. This proves mechanical 340 build composition and ledger adoption, not signing, final-output re-decode/static verification, publication, authenticated authority, or runtime behavior. The 430 real build remains the next fixture gate.

The first approved 430 build attempt from commit `1950343` completed framework installation, decode, and all seven apply operations, then apktool built the intermediate APK successfully. Graft validation failed closed because apktool's added `classes20.dex` carried ZIP flags `0x808` and zero external attributes while Python's seekable `writestr` deterministically emitted flags `0` and safe attributes `0600`; build operation `f17975cffcced3fdabb78951709c3193008b2d2dc57444e91299ef9d13b9adec` quarantined without an effect. Failure marker `/home/arnav/AI/dfinsta-real-build-430-1/failure.json` has SHA-256 `9f6a0fb9fcd6268a46d73f4c4790d43c8f0c818d8f0e15be324ad9528f3a865f`. The reviewed fix canonicalizes only newly added ASCII DEX metadata, requires matching supported input local/central flags, and keeps retained/replacement central metadata and local flags exact against stock. Focused tests pass 62; full warning-strict discovery passes 394 with one expected opt-in skip.

The fresh 430 replay from commit `5388d8062cb986fd2cb455082a9cbc720e0cc940` completed framework installation, decode, all seven operations, stock DEX graft, and clean adoption in 1,457.7 seconds. Canonical evidence `/home/arnav/AI/dfinsta-real-build-430-2/success.json` has SHA-256 `ba16388d7876f5c207694f8fa4e49473d9bcf2f5c479f67d97770857b3fdda78`. Completed build operation `218255a953f9e931e09e58cc002dc0797d0b4ed0a4324210335fa320ec235712` emitted receipt `752994ff2e0a1652a071c804cec1df2e6e6f65f25c809a98d87413ed987205f5`. The final APK is SHA-256 `e2aac4af7c08f7e7d024a9bc477929c5cc4edc447e4d0f28777c4528b66643af`, size 135,648,088, with exact 20-DEX topology, four replacements, added `classes20.dex`, 16,396 retained stock entries, and three stripped signatures. Independent audit verified all 27 reachable producers and final added-DEX local/central flags and mode. Framework/decode/apply/build adoption used the same receipts with zero launches and absent retry workspaces. This proves mechanical 430 build composition and adoption, not final-output re-decode/static verification, signing, publication, authenticated authority, or runtime behavior.

Final-output verification has a separately gate-approved, append-only grant so the canonical replay-v3/build receipts remain unchanged: their original decoder capability admits only stock APKs and cannot be silently reused for final APKs. The unregistered `replay_verify_final_apk_v1` checkpoint requires that exact grant and completed build chain, descriptor-pins the final APK through granted apktool decode, captures final-decoded and exact-source CAS closures, runs the existing static verifier, and emits `ReplayFinalApkVerificationReceiptV1`. Adoption rematerializes those immutable closures in a private workspace, invokes the production receipt validator exactly once, and launches zero subprocesses independent of repository drift. Repeated cancellation waits for decoder cleanup or in-process verification to stop before quarantine. The grant/checkpoint contracts, lifecycle, harness evidence, and smali normalization received independent GO; full warning-strict discovery passes 440 tests with one expected skip.

Real final-verifier evidence now passes for both fixtures. Target 340 from commit `a07c8d46d2504db41eeb94dfad755af8a8f270db` passed 65 assertions and 59 operation proofs; `/home/arnav/AI/dfinsta-real-final-verify-340-3/success.json` has SHA-256 `de879d41f8bab0537ee85343e4f1d427c9c31a35b988ceb20291c268c8c4e1da`. Target 430 from commit `609bacfa35808947644cde904d2cd6b91d83f076` passed 15 assertions and seven operation proofs; `/home/arnav/AI/dfinsta-real-final-verify-430-2/success.json` has SHA-256 `9179519362038c0f410dacbfdffc670a8987447022f522c4eb17fd818b729a0e`. Each adopted the same receipt with one production-validator call, zero launches, and no retry workspace. Fail-closed attempts exposed and corrected only declared apktool cache behavior and semantic smali round-trip normalization; no assertion was waived. Worker/Workflow registration, authenticated authority, signing, publication, and runtime behavior remain pending.
