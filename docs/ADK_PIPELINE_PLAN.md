...
Final-output verification now has a separately gate-approved, append-only grant so the canonical replay-v3/build receipts remain unchanged: their original decoder capability admits only stock APKs and cannot be silently reused for final APKs. The unregistered `replay_verify_final_apk_v1` checkpoint requires that exact grant and completed build chain, descriptor-pins the final APK through granted apktool decode, captures final-decoded and exact-source CAS closures, runs the existing static verifier, and emits `ReplayFinalApkVerificationReceiptV1`. Adoption rematerializes those immutable closures in a private workspace and reruns static verification with zero subprocess launches, independent of repository drift. Repeated cancellation waits for decoder cleanup or in-process verification to stop before quarantine. The grant/checkpoint contracts and lifecycle received independent GO; full warning-strict discovery passes 435 tests with one expected skip. Worker/Workflow registration, signing, publication, and real 340/430 final-verifier evidence remain pending.
