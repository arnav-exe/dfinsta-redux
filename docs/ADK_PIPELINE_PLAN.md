...
The first real 340 build probe then failed closed after apktool exited successfully because the generic executor correctly reported undeclared `framework/1.apk` and `patched-tree/build` outputs. Read-only analysis showed no authoritative patched-tree changes: apktool added exactly `framework/1.apk` plus 15,218 entries below `patched-tree/build`. The reviewed profile policy now admits only those observed build side effects plus `intermediate.apk` for no-framework builds; framework-bearing builds omit `framework/1.apk` authority. The build Activity rejects preexisting generated paths, removes only the admitted cache tree and singly linked framework file descriptor-relatively without following symlinks, preserves declared framework caches, and exact-verifies canonical tree closures before capture. Replacement boundaries fail closed. Full warning-strict discovery passes 391 tests with one expected opt-in integration skip. A fresh real 340 build retry remains pending explicit approval; successful build composition and final APK verification are not yet claimed.
