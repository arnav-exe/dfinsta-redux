...
| 11 Final report | **done** — `dfinsta_pipeline.final_report`; 440 reads 2 of 7 release-ready, 441 reads 4 of 7 from committed data alone. **`expectation.py` is the gate that actually bites**: it derives what a port owes from the previous port's evidence — a set of hook ids, never a number — and exits 3 when one is lost. There is no configured target anywhere, and exactly one way to lower the bar: a recorded human retirement (`dfinsta_pipeline.retirement`), append-only, with `effective_from` derived from the tree so it cannot be backdated onto the port that exposed the drop, and `ruled_by: agent` refused. Un-retirement is another row, so a surface Instagram brings back is expected again without the record losing that it was doubted. Otherwise the only thing that clears a drop is the hook passing again |
