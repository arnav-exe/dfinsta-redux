...
OBSERVE_CLASS = f""".class public final {OBSERVE_DESCRIPTOR[:-1]};
.super Ljava/lang/Object;

# GENERATED by src/dfinsta_pipeline/guards.py. Do not edit by hand.
#
# One line per watched request path, to android.util.Log. It NEVER throws: the
# whole point is to learn which paths the app actually asks for without changing
# what it receives. Unlike the block message, this stays on the device --
# Instagram catches our IOException and files it into its own error event, but it
# has no reason to read our log tag.
...
def write_observe_class(custom_code_root: Path | str) -> Path:
    ...
    destination.write_text(OBSERVE_CLASS, encoding="utf-8")
    return destination
